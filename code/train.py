...
def parser_args():
    ...
    parser.add_argument('--max_length', type=int, default=2048)  # the maximum input sequence length for LLMs
...
def main(**args):
    ...
    current_step = 0
    remove_steps = args['steps_trained']
    for epoch_i in tqdm(range(args['epochs'])):
        ...
        for batch in train_iter:
            ...
            if current_step == length - remove_steps:
                break
        if current_step == length - remove_steps:
            break
